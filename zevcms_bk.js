...
{
  routePath = "/home/zev/apps_nodejs/publisher/";
  console.log("ESTE É O CAMINHHO-> " + routePath);
}
...
app.get('/create', function (req, res) {
    ...
    res.render('structure.ejs', {routePath, page})
...
app.post('/create', function (req, res) {
  ...
  conexao.connect(url, function(err, db) {
      ...
      dbo.collection(collection1).insert({"vtype":""+ vtype +"","brand":""+ brand +"","vclass":""+ vclass +"","model":""+ model+"",
...
app.get('/documento', function (req, res) {
  ...
  conexao.connect(url, function(err, db) {
      ...
      dbo.collection(collection1).find().toArray(function(err, result) {
        ...
        res.render('structure.ejs',{result, routePath, page});
      });
    });
  // End of Read
});
...
app.post('/update', function (req, res) {
  ...
  conexao.connect(url, function(err, db) {
      ...
      var dbo = db.db(database);
      var mongodb = require('mongodb');
      dbo.collection(collection1).update({_id: new mongodb.ObjectID(''+ id +'')},{$set:{"vtype":""+ vtype +"","brand":""+ brand +"","vclass":""+ vclass +"","model":""+ model+"",
        "vversion":""+ vversion+"","vyear":""+ vyear +"",
        "power1":""+ power1+"","torque1":""+ torque1 +"","axis1":""+ axis1 +"","type1":""+ type1 +"",
        "power2":""+ power2+"","torque2":""+ torque2 +"","axis2":""+ axis2 +"","type2":""+ type2 +"",
        "power3":""+ power3+"","torque3":""+ torque3 +"","axis3":""+ axis3 +"","type3":""+ type3 +"",
...
var server = app.listen(21111, function () {
  ...
  var d = Date.now();
    console.log('Servidor node em operação:' + d);
});